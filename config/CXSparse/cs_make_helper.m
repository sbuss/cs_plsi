...
if (isempty (csm))
    ...
    csm = { 'cs_add', 'cs_amd', 'cs_chol', 'cs_cholsol', 'cs_counts', ...
        ...
        'cs_utsolve', 'cs_randperm', 'cs_sqr' ,'cs_plsi', 'cs_plsi_threaded'} ;
        % add cs_mynewfunc to the above list
end
...
%%%%%%%%%%%%%%%%%%%%%%%
% Compile cs_plsi with pthread support
%%%%%%%%%%%%%%%%%%%%%%%
mexcmdp = [mexcmd ' -lpthread'];
[s t kk] = compile_source (srcdir, 'cs_plsi_threaded', obj, hfile, force, mexcmdp, ...
        kk, details) ;
timestamp = max (timestamp, t) ;
anysrc = anysrc | s ;                                                   %#ok
CS = [CS ' ' 'cs_plsi_threaded' obj] ;                                               %#ok
if (nargout > 0)
    objfiles = [objfiles ' ..' filesep 'CSparse' filesep 'cs_plsi_threaded' obj] ;   %#ok
end
